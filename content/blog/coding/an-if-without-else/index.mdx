...
a programmer an `if` and they will response with `else`.
...
I've seen way too many undue usage of `else` and some times `else if`.
It's true, there are cases which warrants the use of `else`, but there are way more
cases which doesn't.
...
That above, may be what you might reach. Let's go one more level
...
Here we have quite a few alterations done since we have first wrote
down the first line of code. What we have achieved is usually what
happens as your requirements change.

Notice that inorder to fulfull our requirements of returning the data,
...
This allows us to avoid creating a local variable and provides clarity. For isntance,
